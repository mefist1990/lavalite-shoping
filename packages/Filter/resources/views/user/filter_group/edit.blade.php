<div class="container-fluid"> 
    <div class="row">
        <div class="col-xs-12">
            <div class="card">
                {!!Form::vertical_open()
                ->id('edit-filter-filter_group')
                ->method('PUT')
                ->files('true')
                ->class('dashboard-form')
                ->action(trans_url(get_guard('url').'/filter/filter_group') .'/'.$filter_group->getRouteKey())!!}
                <div class="header with-sub" data-background-color="red">
                    <div class="row">
                        <div class="col-sm-11 title-main">
                            <i class="pe-7s-news-paper"></i>
                            <h4 class="title">Update FilterGroup</h4>
                            <p class="sub-title">Last updated on {{format_date($filter_group['updated_at'])}}</p>
                        </div>
                        <div class="col-sm-1">
                ...
                <div class="footer">
                    ...
                    <a href="{{ trans_url(get_guard('url').'/filter/filter_group') }}" class="btn-danger btn-raised btn btn-sm" >Cancel</a>
                </div>
                {!! Form::close() !!}
            </div>
        </div>
