...
*/
    $('body').on('click', '[data-action]', function(e) {
        ...
        if ($tag.data('action') == 'CHANGE')
            return app.change($tag.data('value'),$tag.data('field'),$tag.data('href'), $tag.data('load-to'),  $tag.data('datatable'));
...
var app = {
    ...
    'update' : function(forms, tag, datatable) {
     ...
     'change' : function(value,field,url,tag, datatable) {

        var formData = new FormData();
        formData.append(field, value);
        swal({
            title: "Are you sure?",
            text: "This will affected the clients account!",
            type: "warning",
            ...
            $.ajax({
                ...
                {
                    swal("Updation failed!", data.message, "error");
    ...
    'workflow' : function(url, tag, datatable, method, id) {
        ...
        $.each($('.workflow_data'), function(i, val) {
            formData.append(val.name, $('input[name="'+val.name+'"]').val());
        });
